# Event Detection System V2.1.0 - Multi Stream Version 2.0
...
def get_multiple_cctv(limit=4):
    res = supabase.table("cctv") \
        .select("cctv_id, cctv_url, cctv_location") \
        .limit(limit) \
        .execute()
    return res.data
...
def upload_video(local_file_path, bucket_name="evidence_storage"):
    ...
    # Wait a bit to ensure the file is flushed completely
    time.sleep(1)
...
def run_stream(cctv_id, cctv_url, location_name):

    print(f"[START] Monitoring CCTV {cctv_id} - {location_name}")

    cap = cv2.VideoCapture(cctv_url)

    if not cap.isOpened():
        print(f"Error: Could not open stream for CCTV {cctv_id}")
        return

    # Load model inside thread (IMPORTANT)
    model = YOLO('merged.onnx', task="detect")

    # Move ALL per-stream state here
    person_traj = {}
    vehicle_traj = {}
    speed_history = {}
    bbox_history = {}
    ...
    while True:
        ret, frame = cap.read()
        if not ret:
            print(f"Stream disconnected for CCTV {cctv_id}")
            break
        ...
        for box in r.boxes:
            ...
            if w > 0.8 * frame_w or h > 0.8 * frame_h: #if width or height of object is 80% of video frame
                continue

            if cls_id in vehicle_ids or cls_id == 1:   # include bicycle for rider logic
                vehicle_cls[track_id] = cls_id

            color = class_colors.get(cls_id, (255,255,255))
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, box_thickness)
            label = f"{custom_names.get(cls_id,cls_id)} {float(box.conf[0]):.2f} ID:{track_id}"
            ...
            if cls_id == 0:  # person
                ...
                smooth_pt = smooth_traj(person_traj[track_id], TRAJ_WIDNOW)
                pts = person_traj[track_id]
                person_traj[track_id][-1] = smooth_pt  # average out traj for smoother line
                person_bbox[track_id] = (x1, y1, x2, y2)
            elif cls_id in vehicle_ids:
                vehicle_traj.setdefault(track_id, []).append((cx, cy))
                pts = vehicle_traj[track_id]
            ...
            # --- speed
            if len(pts) >= 2:
                v = pixel_speed(pts[-2], pts[-1])
            else:
                v = 0.0
            current_speed[track_id] = v
    ...
    #        cv2.circle(frame, (cx, cy), 4, color, -1)
        ...
        # per-stream histories (kept inside road_state)
        ...
        for vid, traj in vehicle_traj.items():
            ...
            cls_id = vehicle_cls.get(vid)
            if cls_id is None:
                continue

            # rider filter: ignore motorcycle/tricycle/bicycle if someone is riding it
            ...
            for vid2, vbox2 in bbox_history.items():
                ...
                cls2 = vehicle_cls.get(vid2)
                if cls2 is None:
                    continue

                # Optional: skip small-small collisions
            ...
            # fallback: compression + strong-ish drop
            if not accident:
            ...
            # single-vehicle crash fallback ONLY for normal vehicles
            if not accident:
        ...
        if recording:
            ...
            if record_counter <= 0:
                ...
                public_url = event_database_async(record_type, location_name, filename, cctv_id)

        with frame_lock:
            shared_frames[cctv_id] = frame.copy()
...
#    global recording, out, record_type
#    if recording and out is not None:
#        out.release()
#        print(f"Saved clip on exit: {record_type}")
#        try:
#                threat_id, incident_timestamp = event_detection("Human Event", location_name)
#            else:
#                threat_id, incident_timestamp = event_detection("Vehicle Event", location_name)
#
#            public_url = save_videoclip(filename, selected_id, threat_id)
#            threat_alert(threat_id)
#            print(f"Incident uploaded on exit: {public_url}")
#        except Exception as e:
#            print(f"Failed to upload incident on exit: {e}")
# -----------------------------------------------------------------------------
